...
def load_paper_position_snapshot_rows(path: Path) -> list[dict[str, str]]:
...
def load_paper_account_snapshot_rows(path: Path) -> list[dict[str, str]]:
    assert_paper_path(path, PAPER_TEST_DIR)
    if not path.exists():
        raise FileNotFoundError(path)
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
...
def write_paper_symbol_unrealized_performance(rows: list[dict[str, Any]], output_path: Path) -> None:
...
def write_paper_symbol_unrealized_performance_summary(markdown: str, output_path: Path) -> None:
    assert_paper_path(output_path, PAPER_TEST_DIR)
    output_path.parent.mkdir(parents=True, exist_ok=True)
