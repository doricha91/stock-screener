...
def load_paper_symbol_review_bucket_rows(path: Path) -> list[dict[str, str]]:
...
def write_paper_symbol_review_worksheet_csv(rows: list[dict[str, Any]], output_path: Path) -> None:
...
def write_paper_symbol_review_worksheet_markdown(markdown: str, output_path: Path) -> None:
    assert_paper_path(output_path, PAPER_TEST_DIR)
    output_path.parent.mkdir(parents=True, exist_ok=True)
