...
v1.0 이후 완료된 주요 작업은 다음과 같다.

- PAPER16: Daily Ops Status Dashboard closeout 완료
- PAPER17: Export / Sync Policy Hardening closeout 완료
- PAPER18: Alert / Monitoring Report closeout 완료
- PAPER19: Replay / Same-date Diff closeout 완료

v1.1의 핵심 변경점은 다음과 같다.

- 기존 1순위, 1.5순위, 2순위, 2.5순위는 구현/closeout 완료 상태로 갱신한다.
- PAPER19 결과를 반영해 Replay / Same-date Diff를 “최소 하네스”가 아니라 “최소 체인 완료”로 갱신한다.
- PAPER20을 다음 작업으로 신설한다.
- PAPER20의 목적은 Replay Wrapper Operational Smoke / Runbook이다.
...
|---|---|---:|---|
| 0순위 | 다중계좌 구축 환경 점검 | 부분 반영 / 후속 필요 | PAPER16~19에서 account-aware 경로 일부 반영 |
| 1순위 | Daily Ops Status Dashboard | 완료 | PAPER16 |
| 1.5순위 | Export / Sync 정책 정리 | 완료 | PAPER17 |
| 2순위 | Alert / Monitoring Report | 완료 | PAPER18 |
| 2.5순위 | Replay / Same-date Diff 최소 하네스 | 완료 | PAPER19 |
| 2.6순위 | Replay Wrapper Operational Smoke / Runbook | 다음 작업 | PAPER20 |
...
- replay/diff source는 PAPER19에서 별도 진행
...
주요 한계:
- 실제 `paper_sandbox` baseline sidecar 기반 end-to-end smoke는 아직 제한적
...
## 3. 다음 작업: PAPER20 Replay Wrapper Operational Smoke / Runbook

### 3.1. 목적

PAPER20은 기능을 크게 확장하는 작업이 아니라, PAPER19에서 구현한 replay wrapper를 실제 운영에 가까운 방식으로 안전하게 검증하고 운영 절차를 문서화하는 작업이다.

목표:
- 실제 `paper_sandbox` baseline sidecar를 사용해 replay wrapper를 수동 smoke한다.
- `--output-dir` 명시 사용 정책을 정리한다.
- PASS / WARNING / FAIL 결과 해석 runbook을 작성한다.
- 실제 운영 루프에 넣기 전, 사용자가 언제/어떻게 replay diff를 실행해야 하는지 정한다.

### 3.2. 주요 작업 후보

- 실제 `paper_sandbox` baseline sidecar 위치 확인
- 안전한 임시 output dir 또는 검증용 output dir 정책 정의
- `scripts/dev/replay_daily_plan_diff.py` 수동 smoke 절차 작성
- smoke 결과 JSON / Markdown report 확인 방식 정리
- PASS / WARNING / FAIL 해석 기준 작성
- WARNING 발생 시 config_hash / quantity / warning / reason diff를 어떻게 볼지 정리
- 산출물 보관/삭제/무시 기준 정리
- 실제 운영 SOP에 넣을지, 수동 진단 도구로 유지할지 판단

### 3.3. Non-scope

PAPER20에서는 다음을 하지 않는다.
...
- universe/market/indicator fingerprint 구현
- schema/view drift 구현
- 외부 delivery 구현

### 3.4. 성공 기준

- 실제 baseline sidecar 기준 replay wrapper smoke 절차가 정의됨
- `--output-dir` 명시 정책이 정리됨
- PASS / WARNING / FAIL 해석 runbook이 작성됨
- 원장/Notion/actual에 영향 없음
- PAPER19 replay wrapper를 실제 운영자가 안전하게 사용할 수 있는 최소 절차가 생김
...
- PAPER19 replay diff를 전략 변경 검증에 활용
...
2.6. Replay Wrapper Operational Smoke / Runbook — 다음 작업, PAPER20
3. Notion UI 개선 — 후속
...
v1.1 기준으로 Daily Ops Status, Export/Sync 정책, Alert/Monitoring, Replay/Same-date Diff의 최소 기반은 구축됐다.

따라서 다음 단계는 신규 전략 확장이 아니라, PAPER20에서 실제 `paper_sandbox` 기준 replay wrapper operational smoke와 runbook을 통해 PAPER19 기능을 운영 절차로 안전하게 편입하는 것이다.
