...
6-4C can consume a safe preview to prepare the ignored `_runbook_day.local.cmd`. The rollover preview itself remains read-only. Calendar holidays and coverage must be reviewed and extended before the configured coverage end.
