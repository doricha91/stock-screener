...
from core.notion_exporters import (  # noqa: E402
    MANUAL_EXECUTION_TEMPLATE_TARGET,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if export_manual_execution_template and not args.date:
        parser.error("--date is required for --manual-execution-template")
    if export_manual_execution_template and not args.account_id:
        parser.error("--account-id is required for --manual-execution-template")
    ...
    if export_manual_execution_template and not (args.dry_run or args.confirm_actual):
        parser.error("--dry-run or --confirm-actual is required for --manual-execution-template")
    ...
    if export_manual_execution_template and args.dry_run and args.confirm_actual:
        parser.error("--dry-run and --confirm-actual cannot be used together for --manual-execution-template")
    other_targets = [
        export_weekly,
        export_benchmark,
    ...
    if export_daily_ops_status and any([*other_targets, export_manual_execution_template, export_manual_review_template]):
        parser.error("--daily-ops-status cannot be combined with other export targets in this stage")
    if export_manual_execution_template and any([*other_targets, export_daily_ops_status, export_manual_review_template]):
        parser.error("--manual-execution-template cannot be combined with other export targets in this stage")
    if export_manual_review_template and any([*other_targets, export_daily_ops_status, export_manual_execution_template]):
        parser.error("--manual-review-template cannot be combined with other export targets in this stage")
    if not any([*other_targets, export_daily_ops_status, export_manual_execution_template, export_manual_review_template]):
        parser.error(
            "Select at least one target: --weekly, --benchmark, --account-snapshot, --daily-plan, "
            "--daily-review-summary, --daily-ops-status, --manual-execution-template, "
            "--manual-review-template, or --all"
        )
    ...
    if export_manual_execution_template:
        resolved_account_id = normalize_notion_account_id(args.account_id)
        try:
            client = NotionClient(get_notion_token(settings))
            ...
            print("PAPER NOTION EXPORT")
            print(
                f"  manual_execution_template: account_id={resolved_account_id} "
                f"action=failed error={exc}"
            )
            if args.json:
                print(json.dumps(failure_summary, ensure_ascii=False, indent=2))
        ...
        print("PAPER NOTION EXPORT")
        print(
            f"  manual_execution_template: account_id={summary['account_id']} "
            f"execution_date={summary['execution_date']} candidates={summary['candidate_count']} "
            f"create={summary['create_count']} update={summary['update_count']} "
            f"failed={summary['failed_count']} source={summary['source_plan_path']}"
        )
        if args.json:
            print(json.dumps(summary, ensure_ascii=False, indent=2))
        return 0
